	...
	window.addEventListener('message', function (event) {
		...
		switch (event.data.action) {
			...
			case 'updateServerInfo':
				...
				break;
			case 'updateJobGradeAndName':
				if (event.data.jobName) {
					$('#job_name').html(event.data.jobName);
				}
				if (event.data.jobGrade) {
					$('#job_grade').html(event.data.jobGrade);
				}
				if (event.data.name) {
					$('#ooc_name').html(event.data.name);
				}
				break;
			default:
